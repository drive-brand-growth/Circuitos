...
from rag_metrics import QueryLogger, BusinessMetrics
...
def get_query_logger():
    """Get or initialize Query Logger"""
    ...
    if _query_logger is None:
        ...
        try:
            _query_logger = QueryLogger(database_url)
            logger.info("✅ Query Logger initialized")
        except Exception as e:
            logger.warning(f"⚠️ Query Logger disabled: {e}")
            _query_logger = None
    return _query_logger


def get_business_metrics():
    """Get or initialize Business Metrics"""
    ...
    if _business_metrics is None:
        ...
        try:
            _business_metrics = BusinessMetrics(database_url)
            logger.info("✅ Business Metrics initialized")
        except Exception as e:
            logger.warning(f"⚠️ Business Metrics disabled: {e}")
            _business_metrics = None
...
def query():
    ...
    try:
        ...
        # Generate unique query ID for explainability tracking
        query_id = str(uuid.uuid4())
        agent_id = data.get('agent_id')  # Optional: track which agent made the query
        session_id = data.get('session_id')  # Optional: track user session
        ...
        if cached_result:
            ...
            if query_logger:
                ...
                if cached_result.get('results'):
                    query_logger.log_retrieved_chunks(query_id, cached_result['results'])
        ...
        # === STEP 8: Log query for explainability ===
        ...
        if query_logger:
            query_logger.log_query(
                ...
                agent_id=agent_id,
                session_id=session_id,
                relevance_score=quality_check['relevance'].get('score') if quality_check else None,
                quality_gate_passed=quality_check['relevance'].get('relevant') if quality_check else None,
                quality_gate_action=quality_check.get('recommendation') if quality_check else None
            )

            # Log retrieved chunks for explainability
            if results:
                query_logger.log_retrieved_chunks(query_id, results)

        # === Response ===
        response = {
            'query_id': query_id,  # NEW: For linking follow-up events
            'query': query_text,
            'route': route_info['route'],
            'cached': False,
            'results': results,
...
# =============================================================================
# EXPLAINABILITY & ROI METRICS ENDPOINTS (For Investor Pitches)
# =============================================================================

@rag_api.route('/explain/<query_id>', methods=['GET'])
def explain_query(query_id):
    """
    Get detailed explanation for why a query returned specific results.

    NEW FEATURE: Transparency for "Why this answer?" questions.
    ...
    Response:
    {
        ...
        "retrieved_chunks": [
            {
                "chunk_id": "...",
                "document_title": "2026 NPC Better Bodies Classic Rules",
                "document_url": "https://...",
                "text": "...",
                "similarity_score": 0.92,
                "rank": 1,
                "why_selected": "Very high relevance, top result"
            }
        ],
        "business_impact": {
            "high_intent": true,
    ...
    try:
        ...
        if not query_logger:
            return jsonify({
                'success': False,
                'error': 'Query logging is not enabled'
            }), 503

        explanation = query_logger.get_query_explanation(query_id)
        ...
        logger.error(f"Error explaining query {query_id}: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
@rag_api.route('/business_event', methods=['POST'])
def log_business_event():
    """
    Log business events linked to RAG queries (for ROI tracking).

    Request:
    {
        "query_id": "abc-123-...",
        "event_type": "high_intent_detected" | "ghl_lead_created" | "meeting_booked" | "deal_closed",
        "event_value": 50000,  // Optional: revenue in USD
        "metadata": {
            "deal_type": "licensing",
            "source": "agent_qualification"
        }
    }
    ...
    Use cases:
    - Track which queries led to conversions
    - Attribute revenue to RAG system
    - Prove ROI claims to investors
    """
    try:
        data = request.json or {}
        ...
        if not query_id or not event_type:
            return jsonify({
                'success': False,
                'error': 'query_id and event_type are required'
            }), 400

        valid_types = ['high_intent_detected', 'ghl_lead_created', 'meeting_booked', 'deal_closed']
        ...
        query_logger.log_business_event(
            query_id=query_id,
...
@rag_api.route('/roi_dashboard', methods=['GET'])
def roi_dashboard():
    """
    ...
    Response validates investor claims:
    {
        "period_days": 30,
        "query_volume": {
            "total": 1523,
            "cache_hits": 473,
            "cache_hit_rate_pct": 31.05,
            "high_intent": 228,
            "high_intent_rate_pct": 14.97,
            "converted": 34,
            "conversion_rate_pct": 2.23
        },
        "performance": {
            "avg_query_time_ms": 112.5,
            "avg_cached_time_ms": 18.2,
            "avg_uncached_time_ms": 215.3,
            "time_savings_hours": 25.8
        },
        "cost_efficiency": {
            "cache_cost_savings_usd": 189.20,
            "estimated_monthly_savings_usd": 189.20
        },
        "revenue_attribution": {
            "total_revenue_usd": 350000,
            "average_deal_size_usd": 50000,
            "roi_multiple": 1850.3
        },
        "investor_claims_validation": {
            "claim_cache_hit_rate_target": "31%",
            "actual_cache_hit_rate": "31.1%",
            "claim_time_savings_target": "40% faster",
            "actual_time_savings": "91.5%",
            "claim_cost_reduction_target": "25-30%",
            "actual_cost_reduction": "31.0%"
        }
    }
    """
    try:
        ...
        if not business_metrics:
            return jsonify({
                'success': False,
                'error': 'Business metrics are not enabled'
            }), 503

        dashboard = business_metrics.get_roi_dashboard(days)
