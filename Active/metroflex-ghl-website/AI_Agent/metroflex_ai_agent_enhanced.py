...
class MetroFlexAIAgentEnhanced:
    def __init__(self, knowledge_base_path: str, openai_api_key: str):
        ...
        Args:
            knowledge_base_path: Path to METROFLEX_EVENTS_KB_V2_RESEARCH_BASED.json
        ...
        # Conversation memory with user state tracking
        self.conversation_history = {}
        self.user_states = {}  # Track registration flows, lead capture state
    ...
    def _build_vector_database(self):
        ...
        # Index all 2025 events with PRIORITY on dates
        for event_key, event_data in self.knowledge_base['2025_events'].items():
            # Primary event document (date-focused)
            event_text = f"{event_data['official_name'] if 'official_name' in event_data else event_key.replace('_', ' ').title()}: {event_data['date']} ({event_data['day_of_week']}) at {event_data['venue']}, {event_data['location']}. "
            event_text += f"Event type: {event_data['event_type']}. "
        ...
        for package_name, package_data in packages.items():
            package_text = f"{package_name.replace('_', ' ').title()} Sponsorship: ${package_data['price']:,}. Benefits: {', '.join(package_data['benefits'][:3])}..."
...
2. If you don't know something, admit it and offer to connect user with Brian Dobson (brian@metroflexgym.com, 817-465-9331)
3. Always include relevant event dates, registration links, or next steps
4. Reference MetroFlex's 38+ year legacy when appropriate (don't force it)
5. For division questions, be specific about weight classes, height requirements, judging criteria
6. For sponsor inquiries, provide concrete ROI numbers and package pricing
7. For vendor questions, recommend specific services (ProTan for tanning, Physique Visuals for photos)
8. Keep responses concise (2-4 paragraphs max unless detailed explanation needed)
9. End with a clear call-to-action when applicable

**High-Intent Detection:**
When users ask about sponsorship, registration, or vendor booths, you should:
1. Provide the requested information
2. Offer to connect them with the team for personalized follow-up
3. Ask for their contact info (email or phone) so Brian can reach out
